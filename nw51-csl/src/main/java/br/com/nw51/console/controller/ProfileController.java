...
import br.com.nw51.console.services.RoleService;
...
public class ProfileController extends AbstractController implements Serializable {
	...
	@PostConstruct
    public void init() {
		super.checkReadOnly ("papel_perfil_manter");
	...
	public int getProfilesCount() { 
		int count = 0;
		try {
			count = profileService.rowCount();
		} catch (BusinessException e) {
			String error = "An error occurred while getting the row count of profiles. " + e.getMessage();
			logger.error (error);
		}
		return count;
	}
